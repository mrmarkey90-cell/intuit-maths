...
// stored as tenths integers (e.g. 35 = 3.5)
function fmt(v) { return (v / 10).toFixed(1) }
  ...
  return (
    <TapPair
      ...
      genPairs={() => Array.from({ length: 4 }, () => {
        ...
        return shuffle([w1 * 10 + rnd(0, 9), w2 * 10 + rnd(0, 9)])
      })}
      getTarget={(a, b) => Math.max(a, b)}
      doneTip={t('mission.4_2B.bigTip')}
      onNext={onNext}
    />
  )
}

// ── Screen 2: tap smaller — same whole, different decimal digit ───────────────
...
function S2({ onNext }) {
  ...
  return (
    <TapPair
      ...
      doneTip={t('mission.4_2B.smallTip')}
      onNext={onNext}
    />
  )
}
...
export default function Mission4_2B({ pupilId, onComplete }) {
  const [step, setStep] = useState(0)

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '4_2B' })
    setStep(5)
  }
