...
// Both 3-digit; no borrowing — every digit of a is ≥ the matching digit of b.
function genQ() {
  let a, b
  do {
    a = rnd(100, 999)
...
// Chips are 2–3-digit numbers; target is in the hundreds.

function genMiniRound() {
  const target = rnd(150, 350)
  const a = rnd(50, Math.min(200, target - 50))
  const b = target - a
  ...
  return (
    <div className="mission-screen">
      <div className="mission-body">
        ...
        <div style={{ fontSize: 'clamp(44px,11vw,72px)', fontWeight: 900, color: '#0ea5e9', lineHeight: 1, textAlign: 'center', margin: '0.3rem 0 1rem' }}>
          {target}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.7rem', width: 'min(300px,62vw)' }}>
          {chips.map((n, i) => {
            ...
            return (
              <button key={i} className={`mission-bigger-btn${cls}`}
                style={{ fontSize: 'clamp(16px,3.5vw,24px)' }}
                onClick={() => tap(i)} disabled={!!fb}>{n}</button>
            )
          })}
        </div>
...
function S1({ onNext }) {
  ...
  return (
    <div className="mission-screen">
      ...
      <div className="mission-body">
        {done ? (
          <div className="mission-title">{t('mission.4B.unitsFirst3')}</div>
        ) : (
          <>
...
// ── S4: Which has the bigger answer? (column format in buttons) ───────────────
...
export default function Mission4_4B({ pupilId, onComplete }) {
  const [step, setStep] = useState(0)

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '4_4B' })
    setStep(6)
  }
