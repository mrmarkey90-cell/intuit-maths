...
// ── S0: Mini-game — tap two chips that add to a teen number ───────────────────
// Chips from 1–10; target is a teen (11–18).

function genMiniRound() {
  const target = rnd(11, 18)
  const a = rnd(Math.max(1, target - 10), Math.min(10, target - 1))
  const b = target - a
...
function S1({ onNext }) {
  ...
  return (
    <div className="mission-screen">
      ...
      <div className="mission-body">
        {done ? (
          <div className="mission-title">{t('mission.4A.throughTenTip')}</div>
        ) : (
          <>
...
function S2({ onNext }) {
  ...
  return (
    <div className="mission-screen">
      <Progress step={2} />
      <div className="mission-body">
        <div style={{ background: '#f0f2ff', borderRadius: 12, padding: '0.8rem 1.6rem', fontSize: 'clamp(22px,5.5vw,40px)', fontWeight: 700, textAlign: 'center', margin: '0.4rem 0 1rem' }}>
          {a} − {b} = {shown}
        </div>
...
// ── S3: Number bond link — fill in ? + b = a ──────────────────────────────────
// Reframes subtraction as a missing-addend addition problem.

function S3({ onNext }) {
  const { t } = useTranslation()
  const rounds = useMemo(() => Array.from({ length: 3 }, () => {
    const { a, b, diff } = genQ()  // diff is the missing addend
    const opts = new Set([diff])
    for (const off of shuffle([-1, 1, -2, 2, -3, 3])) {
      if (opts.size >= 4) break
      const v = diff + off
    ...
    <div className="mission-screen">
      ...
      <div className="mission-body">
        {done ? (
          ...
          <>
            <div className="mission-subtitle">{t('mission.4A.bondLabel')}</div>
            <div style={{ background: '#f0f2ff', borderRadius: 12, padding: '0.6rem 1.2rem', fontSize: 'clamp(20px,5vw,38px)', fontWeight: 700, textAlign: 'center', margin: '0.3rem 0 1rem' }}>
              <span style={{ color: '#4f46e5', fontSize: 'clamp(24px,6vw,44px)' }}>?</span> + {b} = {a}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem', width: 'min(240px,50vw)' }}>
              {opts.map(opt => (
                <button key={opt}
              ...
              {[q1, q2].map(({ a, b }, idx) => {
                ...
                return (
                  <button key={idx} className={`mission-bigger-btn${cls}`}
                    style={{ flex: 1, maxWidth: 'clamp(100px,24vw,150px)', fontSize: 'clamp(16px,3.5vw,24px)' }}
