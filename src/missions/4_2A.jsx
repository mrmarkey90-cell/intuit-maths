...
// ── Screen 1: compare pairs with different hundreds ───────────────────────────
// e.g. 234 vs 567 — hundreds digit is the decider
...
function S1({ onNext }) {
  ...
  return (
    <Compare2
      ...
      doneTip={t('mission.4_2A.hundredsTip')}
      onNext={onNext}
    />
  )
}

// ── Screen 2: compare pairs with the SAME hundreds digit ─────────────────────
// e.g. 342 vs 378 — tens and units decide
...
// ── Screen 3: fill the gap — count by 50 or 100 ──────────────────────────────

function S3({ onNext }) {
  const { t } = useTranslation()
  const qs = useMemo(() => Array.from({ length: 3 }, () => {
    const step = Math.random() < 0.5 ? 50 : 100
    const start = Math.round(rnd(100, 600) / step) * step
...
// ── Screen 4: drag-sort 4 three-digit numbers ─────────────────────────────────

function S4({ onNext }) {
  const { t } = useTranslation()
  const values = useMemo(() => {
    const zones = [[100, 299], [300, 499], [500, 699], [700, 999]]
  ...
  const chipStyle = { width: 'clamp(50px, 9vw, 70px)', fontSize: 'clamp(12px, 2.3vw, 18px)' }
        ...
        <div className="mission-sort-slots">
          {Array.from({ length: 4 }, (_, i) => (
            <div key={i} ref={el => { slotRefs.current[i] = el }}
              ...
              style={{ width: 'clamp(50px, 9vw, 70px)' }}>
              {placed[i] != null ? (
...
export default function Mission4_2A({ pupilId, onComplete }) {
  const [step, setStep] = useState(0)
  const whereRounds = useMemo(genWhereRounds, [])

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '4_2A' })
    setStep(5)
  }
