...
// stored as hundredths integers (e.g. 345 = 3.45); always show 2dp to reinforce the concept
...
// ── Screen 1: tap bigger — different whole parts ──────────────────────────────
  ...
  return (
    <TapPair
      ...
      genPairs={() => Array.from({ length: 4 }, () => {
        ...
        return shuffle([w1 * 100 + rnd(0, 99), w2 * 100 + rnd(0, 99)])
      })}
      getTarget={(a, b) => Math.max(a, b)}
      doneTip={t('mission.5_2B.bigTip')}
      onNext={onNext}
    />
  )
}

// ── Screen 2: tap smaller — same whole, different tenths/hundredths ───────────
...
function S2({ onNext }) {
  ...
  return (
    <TapPair
      ...
      genPairs={() => Array.from({ length: 4 }, () => {
        const w = rnd(1, 9) * 100
        let a = w + rnd(0, 99), b = w + rnd(0, 99)
        while (a === b) b = w + rnd(0, 99)
        return shuffle([a, b])
      })}
      getTarget={(a, b) => Math.min(a, b)}
      doneTip={t('mission.5_2B.smallTip')}
      onNext={onNext}
    />
  )
}
...
export default function Mission5_2B({ pupilId, onComplete }) {
  const [step, setStep] = useState(0)

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '5_2B' })
    setStep(5)
  }
