...
function genQ() {
  const tens = rnd(1, 9)
  const units = rnd(0, 9)
  return { number: tens * 10 + units, parts: [tens * 10, units] }
}
...
// Box class: default (no --wide) since 2-digit parts (70, 3) fit comfortably.
...
function PartitionInput({ displayNum, parts, labels, hiddenIdxs, onComplete }) {
  ...
  function bCls(i) {
    ...
    if (!hidden) return 'mission-gap-box'
    if (fil || isFlashing) return 'mission-gap-box mission-gap-box--correct'
    if (active) return `mission-gap-box${wrongVal !== null ? ' mission-gap-box--wrong' : ' mission-gap-box--active'}`
    return 'mission-gap-box'
  }
...
// ── Screen 1: Which is bigger? (2-digit) ─────────────────────────────────────
    ...
    <div className="mission-screen">
      ...
      <div className="mission-body">
        <div className="mission-title">
          {done ? t('mission.2_1B.greatComparing') : t('mission.1_1A.whichIsBigger')}
        </div>
          ...
          <div className="mission-partition-boxes">
            {ex.parts.map((p, i) => (
              <Fragment key={i}>
                ...
                >
                  <div className="mission-gap-box">{p}</div>
                  <div className="mission-partition-label">{labels[i]}</div>
                </div>
              </Fragment>
            ))}
...
function S3OneBox({ onNext }) {
  ...
  const questions = useMemo(() =>
    Array.from({ length: 3 }, () => {
      const q = genQ()
      return { ...q, displayNum: String(q.number), hiddenIdxs: [rnd(0, 1)] }
    })
  , [])
  const [idx, setIdx] = useState(0)
  ...
  return (
    <div className="mission-screen">
      <Progress step={3} />
      <div className="mission-body">
        <div className="mission-subtitle">{t('mission.2_1B.fillMissing')}</div>
        <PartitionInput
          key={idx}
...
function S4AllParts({ onNext }) {
  ...
  const questions = useMemo(() =>
    Array.from({ length: 3 }, () => {
      const q = genQ()
      return { ...q, displayNum: String(q.number), hiddenIdxs: [0, 1] }
    })
  , [])
  const [idx, setIdx] = useState(0)
  const q = questions[idx]
  ...
  return (
    <div className="mission-screen">
      <Progress step={4} />
      <div className="mission-body">
        <div className="mission-subtitle">{t('mission.2_1B.fillAll')}</div>
        <PartitionInput
          key={idx}
          displayNum={q.displayNum}
          parts={q.parts}
...
export default function Mission2_1B({ pupilId, onComplete }) {
  const [step, setStep] = useState(0)

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '2_1B' })
    setStep(5)
  }
