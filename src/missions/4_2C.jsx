...
// ── Level 4: round to nearest 1000 (1001–8999) ────────────────────────────────
function loN(n) { return Math.floor(n / 1000) * 1000 }
function hiN(n) { return loN(n) + 1000 }
function roundedN(n) { return Math.round(n / 1000) * 1000 }
function genClear() { let n; do { n = rnd(1001, 8999) } while (n % 1000 === 0 || n % 1000 === 500); return n }
function genAny() { let n; do { n = rnd(1001, 8999) } while (n % 1000 === 0); return n }
const HALFW_POOL = [1500, 2500, 3500, 4500, 5500, 6500, 7500, 8500]
...
function NumberLine({ pct, fmtN, fmtLo, fmtHi }) {
  return (
    <div style={{ width: '100%', maxWidth: 320, margin: '0 auto 1.8rem', padding: '2rem 2.4rem 1.6rem', position: 'relative', boxSizing: 'border-box' }}>
      <div style={{ height: 4, background: '#c0baf0', borderRadius: 2, position: 'relative' }}>
        ...
        <span style={{ position: 'absolute', left: `${pct}%`, top: -28, transform: 'translateX(-50%)', fontWeight: 700, fontSize: 'clamp(11px,2.4vw,16px)', color: '#5b4fe8', whiteSpace: 'nowrap' }}>{fmtN}</span>
        <span style={{ position: 'absolute', left: 0, bottom: -22, transform: 'translateX(-50%)', fontWeight: 600, fontSize: 'clamp(10px,2vw,15px)', whiteSpace: 'nowrap' }}>{fmtLo}</span>
        <span style={{ position: 'absolute', right: 0, bottom: -22, transform: 'translateX(50%)', fontWeight: 600, fontSize: 'clamp(10px,2vw,15px)', whiteSpace: 'nowrap' }}>{fmtHi}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="mission-screen">
      ...
      <div className="mission-body">
        {done ? (
          <RoundingTip lines={t('mission.4_2C.tip').split('|')} example="2800" highlightIndex={1} />
        ) : (
          <>
            ...
            <div className="mission-bigger-row">
              {[[lo_val, fmtLo], [hi_val, fmtHi]].map(([v, label]) => (
                <button
                  ...
                  style={{ fontSize: 'clamp(15px,3vw,24px)', width: 'clamp(80px,16vw,120px)' }}
                  onClick={() => pick(v)}
                  disabled={!!fb}
                >{label}</button>
              ))}
...
function S2({ onNext }) {
  ...
  return (
    <div className="mission-screen">
      <Progress step={2} />
      <div className="mission-body">
        <div style={{ background: '#f0f2ff', borderRadius: 12, padding: '0.8rem 1.4rem', fontSize: 'clamp(16px,3.5vw,26px)', fontWeight: 700, margin: '0.4rem 0 1rem', textAlign: 'center' }}>
...
function S3({ onNext }) {
  ...
  return (
    <div className="mission-screen">
      <Progress step={3} />
      <div className="mission-body">
        <div className="mission-subtitle">{t('mission.4_2C.roundTo1000')}</div>
        <div style={{ fontSize: 'clamp(22px,5vw,38px)', fontWeight: 700, textAlign: 'center', letterSpacing: '0.05em', marginBottom: '0.5rem', color: fb === 'correct' ? '#2e7d32' : fb === 'wrong' ? '#c62828' : 'inherit' }}>
          {String(qs[qi].n)}
        </div>
...
export default function Mission4_2C({ pupilId, onComplete }) {
  const [step, setStep] = useState(0)

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '4_2C' })
    setStep(5)
  }
