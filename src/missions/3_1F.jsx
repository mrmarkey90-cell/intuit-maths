...
// ── Screen 3: True/False — is this in the 2 times table? ─────────────────────
...
function TrueFalseScreen({ step, onDone }) {
  ...
  return (
    <div className="mission-screen">
      ...
      <div className="mission-body">
        <div className="mission-title">
          {done ? t('mission.3_1F.great') : questionText}
        </div>
...
function SpotScreen({ step, findPrompt, onDone }) {
  ...
  return (
    <div className="mission-screen">
      ...
      <div className="mission-body">
        <div className="mission-title">
          {done ? t('mission.3_1F.great') : findPrompt}
        </div>
        <div style={{ visibility: done ? 'hidden' : 'visible', pointerEvents: done ? 'none' : 'auto' }}>
...
export default function Mission3_1F({ pupilId, onComplete }) {
  const { t } = useTranslation()
  const [step, setStep] = useState(0)
  const findPrompt = `${t('mission.1F.findPrefix')}${TABLE}${t('mission.1F.findSuffix')}`

  async function finish() {
    await supabase.rpc('complete_mission', { p_pupil_id: pupilId, p_special_mission: '3_1F' })
    setStep(5)
  }
