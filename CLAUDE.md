...
- 7 interaction types implemented so far (one component per type under `src/insight/modules/`): Numpad (reuses the Instinct `NumberPad` via a popup overlay), Circle (tap-one-of-N), Pair-sum (tap exactly 2 tiles summing to a target — any valid pair counts), Drag-sort (real pointer-based drag via Pointer Events, source row → labelled target slots), Number line (a true ruled line, drag a pin-shaped handle to a notch, only the two end values labelled until marking is revealed — then every notch is labelled), Share (tap +/- to move items from a pool into boxes; counts above 8 collapse to a compact "× N" badge so it never needs scrolling), Fill-blank (a pattern sequence with one gap; drag the correct value up from a row of plausible options into the gap — same Pointer Events approach as Drag-sort, but a single slot inline within the sequence)
