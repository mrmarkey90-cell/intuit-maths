...
- `/hub/[join_code]` — pupil hub (self-select profile, identity check, one-time Setup Test if not yet done, then dashboard: status panel + 4 areas — Special Missions/Games (locked, not built), Practise (expands in place to an Instinct/Insight choice), Into It! (locked banner, future standalone game))
...
- Self-register via `/join/[join_code]` (one link per class, permanent) — creation flow is name + avatar → 5 identity-check questions (one-time) → Setup Test (one-time placement) → Hub
- Join sessions via `/play/[session_code]` (fresh code each challenge) — also gated by the identity check (2 random questions) before joining
...
- 10 questions, all drawn from the real Insight question bank. Adaptive "staircase": starts at level 3, correct → level+1, wrong → level-1 (clamped 1-6). Final level = round(average of the levels the **last 4** questions were asked at), computed client-side.
- Subdomain selection per question excludes a curated denylist (`PLACEMENT_EXCLUDED_SUBDOMAINS` in `src/insight/placementTest.js`) of subdomains judged too easily guessable for a placement test specifically (e.g. binary-choice questions) — this exclusion does not apply to the normal 12-question Insight test or Insight Practice, only here.
...
## Pupil Hub (`/hub/[join_code]`, `src/screens/PupilHub.jsx`)
...
- `generate_join_code()` — shared generator for classes and Instinct sessions: loops a 5-char draw from a 20-letter vowel-free alphabet until `is_clean_code` passes
...
- Pupil profile creation — `/join/[code]` → name + avatar builder → 5 identity-check questions (`SecurityQuestionsSetup`) → Setup Test (`PlacementTest`) → redirected straight into the Hub, pre-selected (no re-tapping their own tile)
...
- **Instinct session (pupil side)** — `/play/[code]` → identity check → lobby wait → 60s question flow with numpad + skip → hyped marking wait → card-based results (score/credits) with Test Level badge, streak dots, comparison vs last time, My Hub button
...
- **Pupil hub** (`/hub/[join_code]`, `src/screens/PupilHub.jsx`) — see "Pupil Hub" above for the current status-panel + 4-area design
