...
- **Arm/leg animation — IK + spring + RAF loop**: all limbs are driven by a single `requestAnimationFrame` loop that mutates SVG `<line>` `x1/y1/x2/y2` attributes directly (same pattern as `FloatingLogos`) — no React state, no 60fps re-renders. The core is a planar two-bone IK solver (`solveIK`) using the law of cosines: given a pivot, a target hand/foot position, and a pole-hint point, it picks whichever of the two valid IK solutions keeps the elbow/knee closer to the pole hint. Arms are additionally run through a 2D damped-spring (`springStep`, K=240, D=22, ζ≈0.71 — slightly underdamped for a springy overshoot on pose transitions); legs use direct IK with no spring, since leg movement is tightly synced to the jump physics and spring lag would look wrong. CSS `@keyframes` arm-sway and wave-wiggle animations were removed entirely and replaced with procedural equivalents in the RAF loop.
- **Poses/state**: `AvatarDisplay` takes an optional `state` prop (`'idle' | 'hips' | 'wave' | 'celebrate'`, plus whatever the future game adds). `POSE_TARGETS` maps each state to per-side `{ hand, pole }` IK targets for the arms; an undefined state falls back to `idle`. When `state` is **not** passed (every screen in the maths platform today), the avatar drives itself via `useAutoState` — occasionally (every ~6-12s, weighted) switching to a brief `hips` or rare `wave` gesture, then back to idle. Once anything passes `state` explicitly, self-gesturing is fully disabled — the caller owns the pose.
- **Celebrate animation**: `celebrate` triggers a looping jump driven by `max(0, sin(tSince × JUMP_FREQ))` (`pumpFraction`) and its mirror `max(0, -sin(...))` (`squatFraction`). The body group receives a `translate(0, bodyY) rotate(…)` transform: `bodyY = -JUMP_HEIGHT × pump + SQUAT_DEPTH × squat` (rises on airborne, lowers during floor pause). Body rotation is `2 × MAX_ROTATION × pump × cos` — zeros at both floor and peak, leans through the arc with no snap; a tiny -3° extra lean during the squat adds anticipation character. Legs lerp between three positions: neutral standing (99% extension, nearly straight) → squat anticipation (80% extension, `squatFraction`) → airborne tuck (knees up and wide, `pumpFraction`). The squat foot target is 7 units higher in local space while the body descends by the same `SQUAT_DEPTH`, so the feet stay at floor level in world space while the knees visibly bend. Arms fist-pump with per-arm phase offsets and a slow drift oscillation so they never feel mechanically in sync.
- **Eyebrows**: procedural paths drawn in hair colour. The `d` attribute is swapped via a `useEffect` on `state` — furrowed/angled-inward during `hips`, neutral arc otherwise. Direct `setAttribute` on a ref, not React state, so no re-render is triggered.
- **Arm rendering history (gotcha)**: the CSS-transform approach for arm rotation was tried first and silently failed for large angles (hips/wave) while appearing to work for small ones (idle sway). Root cause never pinned down; replaced with plain JS trigonometry into absolute SVG coordinates, which has no such failure mode. Arms are painted after the head/hair in the SVG so raised arms on `wave`/`celebrate` appear in front of the face rather than behind it.
...
- A flex row with `align-items: center` sizes each child to its own *content* height rather than stretching it to the row's bounded height — fine on a normally-scrolling page, but inside a `.pupil-viewport` screen (locked height, `overflow: hidden`) a child whose content is taller than the row silently overflows the row and gets clipped equally off the top *and* bottom by the screen's `overflow: hidden`, with no scrollbar and no visual cue anything's missing. Caught on `PupilProfileCreate.jsx`'s original avatar+form split on a short landscape phone — first/last name and the submit button were unreachable. First fix attempt was `align-items: stretch` plus `overflow-y: auto` as a fallback — technically reachable, but produced a visible scrollbar on a screen that's supposed to feel like a single fixed-size kids' UI, which was worse than the bug. Real fix was reducing how much content needed to fit at all: split into `.profile-left-col` (name inputs + avatar preview + submit) / `.profile-right-col` (avatar-builder controls) as two genuinely separate columns instead of one tall stack, and dropped the title/tagline above it as redundant chrome. Both columns now use `overflow: hidden` (not `auto`) deliberately — there's no scroll escape hatch, so content fitting via clamp sizing is load-bearing, not optional. Any other side-by-side `.pupil-viewport` split (question+numpad, etc.) that uses `align-items: center` on the row is at risk of the same silent clipping if its content column grows; prefer narrowing/splitting the content over reaching for `overflow: auto`.
