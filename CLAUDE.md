...
**Current implementation status: dev test harness only.** `/insight-test` (`src/screens/InsightTest.jsx`) lets you pick a domain/level and answer one module, or run a full 12-question carousel with Submit → marking → results → review. It does **not** call `submit_insight_attempt` or write real data — results/credits shown there are client-side mocks. Levels 1-5 generators exist (`src/insight/generators/level1.js` through `level5.js`); Level 6 is not yet written. 3A at Level 2 deliberately reuses `L1_3A` directly (the sheet specifies an identical question), rather than duplicating it into `level2.js`; similarly Level 5's 4C reuses `level4.L4_4C` directly — that generator was built for Level 4 but deliberately held back to Level 5 for difficulty reasons (see gotcha below). 8B (Ratios) is deliberately unimplemented at every level — kept in `domainConfig.js` so the code still shows it as "Coming soon", per explicit instruction not to build it. `NumpadModule` supports an optional `column` layout (`{ a, b, operator }`) that renders the problem as a real written column instead of inline text, used for column addition/subtraction. The real pupil-facing flow (PupilHub integration, weekly gating, actual DB submission) is not yet built.

**`domainConfig.js` open question:** the Level 5 curriculum list showed "—" for 4B, 5C and 6D, all of which my config currently marks active through Level 6 — unclear whether these were deliberately deleted subdomains (the user mentioned deleting some this round) or just paused for Level 5 specifically. Left untouched pending confirmation; generators simply aren't registered for them at Level 5, so they show "Coming soon" either way.
