...
- A flex row with `align-items: center` sizes each child to its own *content* height rather than stretching it to the row's bounded height — fine on a normally-scrolling page, but inside a `.pupil-viewport` screen (locked height, `overflow: hidden`) a child whose content is taller than the row silently overflows the row and gets clipped equally off the top *and* bottom by the screen's `overflow: hidden`, with no scrollbar and no visual cue anything's missing. Caught on `PupilProfileCreate.jsx`'s avatar+form split on a short landscape phone — first/last name and the submit button were unreachable. Fixed by using `align-items: stretch` on the row instead (so the form column gets a real, bounded height) plus `overflow-y: auto` on that column as a guaranteed fallback if tightened clamp sizing still doesn't fit on some device. Any other side-by-side `.pupil-viewport` split (question+numpad, avatar+form, etc.) that uses `align-items: center` on the row is at risk of the same silent clipping if its content column grows.
