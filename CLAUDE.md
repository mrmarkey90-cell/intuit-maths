...
- Move up: 10 correct answers in a session counts as a streak tick (`challenge_streak` +1); 3 consecutive ticks → stage + 1, streak resets to 0
...
- `get_class_pupils(join_code)` — public/anon, returns pupil list (id, first_name, last_name, avatar, current_stage, challenge_streak)
...
- `get_pupil_history(pupil_id)` — anon, returns `{ pupil: {...}, attempts: [...] }` — last 10 challenge attempts with stage/score/total/date
...
4. Reporting views (leadership — whole-school session history and pupil progress)
